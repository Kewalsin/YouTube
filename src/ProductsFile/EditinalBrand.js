...
import WebView from 'react-native-webview';
 const EditinalBrand = ({setIsLoading}) => {
  ...
  const FetchApiData= async()=>{
    try {
      setIsLoading(true)
        ...
        if(status===200){
            setVideoData(data.data["EDITORIAL/BRANDED"])
        }
  ...
  return (
    <>
    {
      showHeader && (
        ...
        <Text style={[styles.showMoreText,{fontSize:20,borderBottomWidth:1,borderColor:"#FFF"}]}>EDITORIAL/BRANDED</Text>
    </View>
      )
    }
...
const styles = StyleSheet.create({
    ...
    mainView: {
      ...
      video:{
        ...
        ,backgroundColor:"#333",borderWidth:1,borderColor:"#FFF"
      }
})
